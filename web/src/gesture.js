...
const SWIPE_UPRIGHT_FRAMES       = 2;   // consecutive upright frames required before tracking begins
...
function updateSwipe(side, hs, lms, mpConf, now) {
  ...
  if (sw.state === "IDLE") {
    ...
    if (!gatePassed) {
      sw.uprightFrames = 0;
      dbg(`[SWIPE] uprightness: ${uprightness.toFixed(3)}, passed: false, consecutive: 0/${SWIPE_UPRIGHT_FRAMES}`);
      return null;
    }
    ...
    sw.uprightFrames++;
    dbg(`[SWIPE] uprightness: ${uprightness.toFixed(3)}, passed: true, consecutive: ${sw.uprightFrames}/${SWIPE_UPRIGHT_FRAMES}`);
  ...
  if (sw.state === "TRACKING") {
    ...
    // must reach its own minimum AND the x-component must be at least 40% of total
...
export function proposeGestureFromLandmarks(results) {
  ...
  // Global cooldown — block all proposals for COOLDOWN_MS after the last one
  ...
  if (cooldownRemaining > 0) {
    ...
    // Read-only uprightness peek — no state mutation, just logging
    for (let _i = 0; _i < (results.multiHandLandmarks || []).length; _i++) {
      const _lms = results.multiHandLandmarks[_i];
      ...
      dbg(`[SWIPE] uprightness: ${_u.toFixed(3)}, passed: ${_passed}, consecutive: ${_frames}/${SWIPE_UPRIGHT_FRAMES} [cooldown]`);
    }
    return null;
  }
